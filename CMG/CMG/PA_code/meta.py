...
def get_params():
    ...
    parser.add_argument("--seed", type=int, default=714755, help='random seed')
    ...
    parser.add_argument("--pos_num", type=int, default=4, help='patch size')
    parser.add_argument("--neg_num", type=int, default=14, help='patch size')
    ...
    for episode in range(params['episode']):
        for inner in range(params['inner_num']):
            ...
            for i in range(params['K']):
                ...
                loss.append(
                    ce(nn.Softmax(1)(logits), train_s_l.long().reshape(-1, ))+params['lamda1']*contrastive(z_v_,z_cp,train_s_l.long().reshape(-1, ),params))
                z_v_list.append(z_v_)


            for j in range(params['K']):
                z_v_all=torch.cat(z_v_list,0)
                loss_total = loss_total + loss[j]+params['lamda']*((contrastive(z_v_list[j],z_cp,train_s_l.long().reshape(-1, ),params)-contrastive(z_v_all,z_cp,train_s_l.long().reshape(-1, ),params)))**2
            ...
            if inner == 0:
                with torch.no_grad():
                    grads_f = torch.autograd.grad(loss, B_vae.c_encoder.parameters(), create_graph=True, allow_unused=True)
        ...
        if episode == 0 or episode % 10 == 0:
            ...
            if use_parameters==True:
                   feture_encoder.load_state_dict(torch.load("./checkpoints_PA/parameters/B_VAE_finally.pkl"))
                  B_vae.load_state_dict(torch.load("./checkpoints_PA/parameters/B_VAE_finally.pkl"))
            ...
            with torch.no_grad():
                print('testing------------------------------------------')
                #若显存不足，将use_sub设置为True
                use_sub=False
                if use_sub:
                  sub_num=20000 #自定义数量，子集样本数量越多，性能越高
                  rand_choose=randonm.sample(range(0,data_s.shape[0]),data_s.shape[0])[:sub_num]
                  feture = feture_encoder(torch.tensor(data_s[rand_choose]).cuda().reshape(-1, params['patches'] ** 2, b_s))
                  _, _, z_c_s = B_vae(feture, label=1)
                  KNN_classifier = KNeighborsClassifier(n_neighbors=params['n_neighbors'], p=2)
                  KNN_classifier.fit(z_c_s.cpu().detach().numpy(), label_s[rand_choose])
                else:
